def register():
    ...
    if a in d:
        ...
        print("enter username in a proper email format")
        register()

    elif ((a.index('@')) - (a.index('.'))) == 1:
        print("Enter your email to register please!")
        register()
    ...
    b = input("Create your password with atleast one upper case letter  one number and one special character: ")
    s = False
    ...
    file = open("python_register.txt", "a")
    file.write(a + "," + b + "\n")
    file.close()
    login()
...
def login():
    ...
    if X in d:
        ...
        Y=Y.strip()
        file1=open("python_register.txt","r").readlines()
        for x in file1:
            x=x.strip()
            info=x.split(",")
            if X==info[0] and Y==info[1]:
                ...
                if F == "Y":
                    ...
                    file = open("python_register.txt", "w")
                    file.write(X + "," + b + "\n")
                    file.close()
...
def welcome():
    print("WELCOME TO THE PYTHON REGISTRATION PORTAL")
    ...
    if W=="L":
        ...
        print("Enter  L or R only please")
        welcome()
welcome()
